...
public class PluginMessageListener {
    public final MinecraftChannelIdentifier IDENTIFIER = MinecraftChannelIdentifier.from("multilogincommand:main");
    private final List<MessageHandler> handlers = new ArrayList<>();
    private static volatile PluginMessageListener instance;

    public void registerHandler(MessageHandler handler) {
        handlers.add(handler);
        MLCommandVelocity.getInstance().getLogger().info("Registered handler: {}", handler.getClass().getName());
    }
